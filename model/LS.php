  ...
  public static function register( $id, $password, $other = array() ){
    ...
    if( self::userExists($id) || (isset($other['email']) && self::userExists($other['email'])) ){
      ...
      if( count($other) == 0 ){
        ...
        $sql = self::$dbh->prepare("INSERT INTO `". self::$config['db']['table'] ."` (`username`, `password`, `profile_img`) VALUES(:username, :password, :profileImg)");
        ...
        $sql   = self::$dbh->prepare("INSERT INTO `". self::$config['db']['table'] ."` (`username`, `password`, $columns, `profile_img`) VALUES(:username, :password, :$colVals, :profileImg)");
      ...
      $newid = self::$dbh->lastInsertId();
      $path = "users/" . $newid."/";
      if(!is_dir($path)){
        mkdir($path, 0777);
        mkdir($path."/main/", 0777);
